...
export const projects: Project[] = [
  {
    ...
    highlights: [
      "문제: 노후화된 업무 화면으로 인한 사용자 작업 동선 비효율 및 대규모 함정 운용·정비 데이터 누적에 따른 조회 지연",
      "해결: 사용자 중심의 웹 UI 개편 및 업무 편의 기능 추가 개발, MSSQL 복합 인덱스 최적화 및 C#/ASP.NET 백엔드 트랜잭션 튜닝",
      "성과: 직책별 접근 제어(RBAC)를 통한 보안성 강화와 무결함 데이터 처리를 달성하여 현업 운용 효율성 대폭 향상",
    ],
    features: [
      "직책 및 보안 등급에 따른 세분화된 접근 제어(RBAC) 및 보안 비즈니스 로직",
      "대규모 함정 운용·정비·장비 상태 데이터 집계 및 다각도 조건부 복합 조회 화면",
      "현업 업무 동선 최적화를 위한 관리자 웹 UI 개편 및 데이터 그리드 편의 기능 고도화",
      "제약된 폐쇄망 인프라 환경 내 안정적 운영을 위한 트랜잭션 예외 처리 및 장애 모듈 개선",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "성과: 지문 탐색 딜레이 0초 달성 및 AI 인식 오차를 즉시 교정하는 Human-in-the-loop(±0.5초 보정) 인터페이스 구축",
    ],
    features: [
      "Whisper AI 기반 음성 전사(Transcription) 및 문항별 시작/종료 타임스탬프 자동 구조화",
      "문항 트리(Q1-3, Q4-6 등) 클릭 즉시 딜레이 없는 0초 칼시작 재생",
      "0.1초 단위 구간 미세 조정 패널(±0.5s 보정, 초기화, 로컬 영구 저장)",
      "재생 위치 기반 실시간 대본 하이라이트 추적 및 더블 클릭 인라인 텍스트 수정",
      "교재 및 PDF 뷰어와 병행 학습할 수 있는 F10 초슬림 플로팅 미니 플레이어 뷰",
    ],
    stack: [
      "OpenAI Whisper",
      "Python",
  ...
  },
  {
    ...
    features: [
      "아침/저녁 컨디션 1회 탭 체크인 및 텍스트 없는 즉각적 공감 칩 피드",
      "시간대 및 사용자 상태에 따른 동적 테마 톤앤매너 매핑",
      "6자리 초대 코드 및 클립보드 자동 감지 기반의 간편 온보딩/그룹 참여",
      "방장 전용 멤버 관리 제스처(카드 롱클릭 내보내기 다이얼로그 + 햅틱 피드백)",
      "FCM 원터치 긴급 상황 전파 및 Cloud Functions 비동기 무결성 관리",
    ],
    stack: [
      "Kotlin",
      "Android",
  ...
  },
  {
    ...
    features: [
      "Android 금융 알림(SMS/푸시) 실시간 수신 Webhook 엔드포인트 및 보안 토큰 인증",
      "복합 상호명, 특수문자, 조사를 분해·정규화하는 결제 파서 엔진 및 디바운스 방어",
      "Supabase Cron 연동 '1일 1회 정각' 서버 주도형 Web Push 일일 리마인더 파이프라인",
      "Next.js App Router 기반 월별 예산 임계치 실시간 계산 대시보드 및 지출 통계 시각화",
      "반복 결제(구독) 관리 및 수신함(Inbox) 트랜잭션 승인/거절 플로우",
    ],
    stack: [
      "Next.js",
      "TypeScript",
